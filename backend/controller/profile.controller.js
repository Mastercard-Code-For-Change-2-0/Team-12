...
const profile = async (req, res) => {
  try {
    ...
    if (!userProfile) {
      ...
      return res.status(404).json({ message: "aaaaaaaaaaa User profile not found" });
    }
...
const deleteProfile = async(req , res ) => {
    try {
        ...
        const userData = await UserData.findOneAndDelete({ username });
        if (!user) {
            console.log("User profile not found for username:", username);
...
export { updateProfile, profile, deleteProfile };
